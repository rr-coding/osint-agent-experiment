"""
Tool definitions and implementations for the British Army OSINT agent.
"""
import json
import time
import requests
...
TOOL_SCHEMAS = [
    {
        "name": "web_search",
        "description": (
            "Search the web for information about British Army units, deployments, and bases. "
            ...
            "Fan out across many narrow queries rather than one broad compound query."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "site": {
                    "type": "string",
                    "description": "Optional: restrict to a specific domain, e.g. 'army.mod.uk'.",
                },
            },
            "required": ["query"],
        },
    ...
    },
    {
        ...
        "description": (
            "Extract British Army unit information from raw text content using AI. "
            "Identifies regiments, battalions, brigades, bases, and their locations. "
    ...
    },
    {
        ...
        "description": (
            "Resolve a vague location description for a British Army unit into approximate "
        ...
        "input_schema": {
            ...
            "properties": {
                "asset_name": {
                    "type": "string",
                    "description": "Name of the British Army unit (e.g., '1st Battalion Royal Welsh')",
                },
                "location_description": {
                    "type": "string",
                    "description": "Location description to resolve",
                },
            },
    ...
    },
    {
        "name": "task_complete",
        "description": (
            "Signal that the intelligence gathering task is complete."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
...
def tool_extract_and_update(client: anthropic.Anthropic, text: str, source_url: str) -> dict:
    """Use Claude to extract British Army units from text."""
    today = datetime.utcnow().strftime("%Y-%m-%d")
    prompt = f"""You are an OSINT analyst specialising in British Army order of battle.

Extract ALL British Army units mentioned in the following text. For each unit, identify:
- name: the full name (e.g., "1st Battalion Grenadier Guards", "Catterick Garrison", "16 Air Assault Brigade")
- type: one of regiment, battalion, brigade, division, barracks, training_establishment, headquarters, formation, other
- class: the unit type/description if mentioned (e.g., "Light Cavalry", "Infantry", "Armoured")
- location_description: any location information mentioned (garrison, country, exercise area, etc.)
- date_observed: the date this information relates to (YYYY-MM-DD format, use {today} if current)

Only include CONFIRMED British Army (not RAF, Royal Navy, allied) units.
If no British Army units are found, return an empty list.
...
Respond ONLY with valid JSON — a list of unit objects. Example format:
[
  {{
    "name": "1st Battalion Royal Welsh",
    "type": "battalion",
    "class": "Infantry",
    "location_description": "Tidworth",
