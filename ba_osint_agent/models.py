...
ArmyUnitType = Literal[
    "regiment", "battalion", "brigade", "division",
    "barracks", "training_establishment", "headquarters", "formation", "other"
]


class ArmyAsset(BaseModel):
    name: str
    type: ArmyUnitType
...
class AgentState(BaseModel):
    assets: List[ArmyAsset] = Field(default_factory=list)
    ...
    def assets_with_locations(self) -> List[ArmyAsset]:
        return [a for a in self.assets if a.has_location()]

    def assets_without_locations(self) -> List[ArmyAsset]:
        return [a for a in self.assets if not a.has_location()]

    def get_asset_by_name(self, name: str) -> Optional[ArmyAsset]:
        for asset in self.assets:
            if asset.name.lower() == name.lower():
                return asset
        return None

    def upsert_asset(self, new_asset: ArmyAsset) -> bool:
