...
AssetType = Literal["ship", "submarine", "base", "air_station", "unit", "auxiliary"]


class NavyAsset(BaseModel):
    name: str
    type: AssetType
...
class AgentState(BaseModel):
    assets: List[NavyAsset] = Field(default_factory=list)
    ...
    def assets_with_locations(self) -> List[NavyAsset]:
        return [a for a in self.assets if a.has_location()]

    def assets_without_locations(self) -> List[NavyAsset]:
        return [a for a in self.assets if not a.has_location()]

    def get_asset_by_name(self, name: str) -> Optional[NavyAsset]:
        for asset in self.assets:
            if asset.name.lower() == name.lower():
                return asset
        return None

    def upsert_asset(self, new_asset: NavyAsset) -> bool:
        ...
        if existing:
            ...
            # Accept new coordinates if: existing has none, OR new confidence is higher
