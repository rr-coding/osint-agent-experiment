"""
Royal Air Force OSINT Tracking Agent
=====================================
A genuine AI agent that uses the Anthropic API with tool use to dynamically
gather open-source intelligence about RAF squadrons, stations, and formations.
"""
...
import config
from models import AgentState, RAFAsset
from tools import TOOL_SCHEMAS, execute_tool
from geocoding import resolve_location

console = Console()

SYSTEM_PROMPT = """You are an OSINT analyst specialising in Royal Air Force order of battle and unit dispositions.

Your GOAL is to identify Royal Air Force units (squadrons, stations, wings, groups, flights) and their current or most recently reported locations from publicly available open sources.

STRATEGY:
1. Search for recent RAF news, deployments, exercises, and scrambles
...
5. For units with no location description, search specifically for their home station
6. Fixed locations (RAF stations) should be resolved immediately from the name alone

UNIT SEARCH GUIDANCE:
- Search for squadrons one at a time with narrow queries: '"IX Squadron" Marham 2026' not multiple squadrons at once
- Named operations (Op Shader, Op Biloxi) are strong location indicators
- QRA scrambles often name the station (Coningsby, Lossiemouth)
- Exercise deployments (Red Flag, Cobra Warrior, Cope North) name the location

LOCATION PRIORITY RULES:
- RAF stations (Marham, Coningsby, Lossiemouth, Brize Norton, etc.): use fixed station coordinates
- Units "at Akrotiri" or "Cyprus": use Akrotiri coordinates
- Units "Op Shader": use Akrotiri coordinates (the Op Shader hub)
- Units deployed to specific overseas bases: use that base's coordinates
...
- Keep queries NARROW: '"11 Squadron" Coningsby' beats '"11 Squadron" OR "29 Squadron" deployment operations 2026'.
...
- Search results give only titles, URLs and snippets. When a result looks relevant, call fetch_page to read the full article.
...
def build_state_context(state: AgentState) -> str:
    ...
    if state.assets_without_locations():
        lines.append("UNITS MISSING LOCATIONS (PRIORITY):")
            ...
            for asset_raw in result.get("assets", []):
                try:
                    asset = RAFAsset(
                        ...
                        type=asset_raw.get("type", "squadron"),
                    ...
                    if live_asset and live_asset.location_description and live_asset.latitude is None:
                        console.print(f"[dim]  Auto-geocoding: {live_asset.name} — '{live_asset.location_description}'[/dim]")
                        try:
            ...
            if lat is not None and lon is not None:
                ...
                if asset:
                    ...
                    if asset.latitude is None or conf > existing_conf:
                        ...
                        console.print(
                            f"[green]  📍 Located: {asset_name} → [{lat:.2f}, {lon:.2f}] (conf={conf:.2f})[/green]"
                        )

    # Sweep: auto-geocode any units with description but no coords
    for asset in state.assets_without_locations():
        if asset.location_description:
            console.print(f"[dim]  Sweep-geocoding: {asset.name} — '{asset.location_description}'[/dim]")
            try:
                ...
                if lat is not None and lon is not None:
                    ...
                    asset.last_updated = datetime.now(timezone.utc).isoformat() + "Z"
                    console.print(
                        f"[green]  📍 Sweep-located: {asset.name} → [{lat:.2f}, {lon:.2f}] (conf={conf:.2f})[/green]"
                    )
            except Exception as geo_err:
...
def run_agent():
    console.print(Panel.fit(
        "[bold blue]Royal Air Force OSINT Tracking Agent[/bold blue]\n"
    ...
    initial_message = (
        "You are starting a Royal Air Force OSINT collection mission.\n\n"
        + build_state_context(state)
        + "\n\nBegin by searching for recent RAF news, deployments, and exercises. "
        "Search for squadrons one at a time with narrow, focused queries.\n\n"
    ...
    for iteration in range(1, config.MAX_ITERATIONS + 1):
        ...
        if len(state.assets_with_locations()) >= config.TARGET_ASSETS_WITH_LOCATIONS:
            console.print(f"[green bold]✓ Target reached: {len(state.assets_with_locations())} units with locations![/green bold]")
            state.complete = True
        ...
        if iteration > 1:
            messages.append({
                ...
                "content": (
                    ...
                    "Avoid repeating searches already run."
                ),
            })
    ...
    table = Table(title="Discovered RAF Units", show_header=True, header_style="bold magenta")
    ...
    for asset in state.assets:
        ...
        table.add_row(asset.name, asset.type, loc_str[:30], Text(conf_str, style=conf_style))
    ...
    output_path = output_dir / f"raf_assets_{timestamp}.json"
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.status_code >= 500:
                last_exc = e
                delay = min(2 ** attempt + random.uniform(0, 1), 60)
                console.print(f"[yellow]Server error. Retry {attempt + 1}/{max_retries} in {delay:.1f}s[/yellow]")
                time.sleep(delay)
            else:
                raise
    raise last_exc
