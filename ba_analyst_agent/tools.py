"""Tool definitions and implementations for the BA Analyst Agent."""
import json
import requests
...
TOOL_SCHEMAS = [
    {
        "name": "web_search",
        "description": (
            "Search the web for information about a British Army unit's current status, "
            "deployment, exercises, or assignment."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            "Generate the master list of operation types relevant to this British Army dataset. "
            "Call this ONCE at the start, passing a summary of all units."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "assets_summary": {
                    "type": "string",
                    "description": "A summary of all units in the dataset",
                }
            },
    ...
    },
    {
        "name": "assess_asset",
        "description": (
            "Assess a single British Army unit and return full enrichment: "
            "operational readiness, current assignment, unit category, capability scores."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "asset_name": {"type": "string", "description": "Full name of the unit"},
                "asset_class": {"type": "string", "description": "Class/type of the unit"},
                "asset_type": {"type": "string", "description": "Type from OSINT data"},
                "location_description": {"type": "string", "description": "Last known location"},
                "gathered_context": {"type": "string", "description": "Context from web searches"},
                "operation_types": {
                    "type": "array",
                    "items": {"type": "string"},
...
def tool_generate_operation_types(client: anthropic.Anthropic, assets_summary: str) -> dict:
    prompt = f"""You are a British Army intelligence analyst. Examine the following list of British Army units and generate up to {config.MAX_OPERATION_TYPES} operation types that are relevant and meaningful for THIS specific set of units.

Units in the dataset:
{assets_summary}

Guidance: Consider Army-relevant operation categories such as armoured warfare, infantry operations, artillery support, combat engineering, reconnaissance, peacekeeping and stabilisation, training and mentoring of partner forces, counter-terrorism, humanitarian assistance, and logistics/sustainment. Generate only the types that are genuinely applicable to the units present.
...
[
  {{
    ...
    "description": "Offensive and defensive operations employing armoured vehicles and combined arms"
  }}
]"""
    try:
...
def tool_assess_asset(
    ...
    prompt = f"""You are a British Army intelligence analyst. Assess the following British Army unit and return a structured JSON enrichment.

Unit: {asset_name}
Type/Class: {asset_class or asset_type or 'Unknown'}
Last known location: {location_description or 'Unknown'}
Today's date: {today}

Gathered intelligence context:
---
{gathered_context[:3500] if gathered_context else 'No specific intelligence gathered. Use general knowledge about this unit type.'}
---
...
{{
  "unit_category": "<descriptive category, e.g. 'Light Infantry', 'Armoured', 'Artillery', 'Combat Engineer', 'Reconnaissance', 'Logistics', 'Special Forces', 'Training', 'Garrison', 'Headquarters', etc.>",
  "operational_readiness": "<high, medium, or low>",
  "readiness_rationale": "<explain your reasoning>",
  "current_assignment": "<current operation/exercise/garrison, or 'No current assignment identified'>",
  ...
  "parent_brigade": "<parent brigade if known, otherwise null>",
  "regimental_identity": "<regiment or corps affiliation if applicable, otherwise null>",
  "vehicle_fleet": "<primary vehicle/equipment platform if known, otherwise null>"
}}

Scoring guide: 0=not applicable, 1=minimal, 2=limited, 3=moderate, 4=strong, 5=primary role/optimised.
If readiness is uncertain due to limited information, default to medium and say so in the rationale."""
