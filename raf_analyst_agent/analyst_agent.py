"""
Royal Air Force Analyst Agent
==============================
Takes the JSON output from the RAF OSINT tracking agent and enriches each unit
with operational readiness, current assignment, unit category, and capability scores.

Usage:
    python analyst_agent.py path/to/raf_assets_TIMESTAMP.json
"""
...
SYSTEM_PROMPT = """You are a Royal Air Force intelligence analyst assessing the current order of battle. You have been given a dataset of RAF units with their names, types, aircraft, and last known locations.

Your task is to enrich this dataset with analytical assessments. For each unit, determine its operational readiness, current assignment, precise unit category, and its capability to support different types of air operations.

Use web search tools to find current information about each unit's status — deployments, exercises, QRA commitments, and operational assignments. For general knowledge about aircraft capabilities and squadron roles, use your training knowledge.

Be honest about uncertainty. If you cannot determine a unit's current assignment, say so. Do not fabricate deployments.

Think about the units as a force. When you research one squadron and discover information about others at the same station or in the same operation, use that information across multiple assessments.
...
def build_assets_summary(raw_assets: List[dict]) -> str:
    ...
    for a in raw_assets:
        cls = a.get("class") or "aircraft unknown"
...
def apply_assessment(enriched: List[EnrichedAsset], asset_name: str, assessment: dict):
    ...
    asset.aircraft_type = assessment.get("aircraft_type")
    asset.parent_wing = assessment.get("parent_wing")
    asset.squadron_number = assessment.get("squadron_number")
...
def run_analyst(input_path: str):
    console.print(Panel.fit(
        "[bold blue]Royal Air Force Analyst Agent[/bold blue]\n"
    ...
    for raw in raw_assets:
        try:
            asset = EnrichedAsset(
                ...
                service=raw.get("service", "royal_air_force"),
                type=raw.get("type", "squadron"),
                **{"class": raw.get("class")},
    ...
    initial_msg = (
        "You are starting a Royal Air Force analytical assessment.\n\n"
        f"{assets_summary}\n\n"
        "Step 1: Call generate_operation_types with the units summary above.\n"
        "Step 2: Research and assess each unit, then call assess_asset.\n"
    ...
    table = Table(title="RAF Unit Assessments", show_header=True, header_style="bold magenta")
    ...
    out_path = output_dir / f"raf_enriched_{timestamp}.json"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        console.print(
            "[bold]Usage:[/bold] python analyst_agent.py path/to/raf_assets_TIMESTAMP.json\n"
            "[dim]Example:[/dim] python analyst_agent.py "
            "../raf_osint_agent/output/raf_assets_20260412_120000.json"
        )
        sys.exit(1)
