...
class EnrichedAsset(BaseModel):
    ...
    # ── RAF-specific optional fields ───────────────────────────────────────────
    aircraft_type: Optional[str] = None
    parent_wing: Optional[str] = None
    squadron_number: Optional[str] = None
    ...
    def to_output_dict(self) -> dict:
        ...
        if self.aircraft_type:
            d["aircraft_type"] = self.aircraft_type
        if self.parent_wing:
            d["parent_wing"] = self.parent_wing
        if self.squadron_number:
            d["squadron_number"] = self.squadron_number
        return d
